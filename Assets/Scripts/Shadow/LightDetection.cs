...
namespace Shadow
{
    [RequireComponent(typeof(MeshBend))]
    public class LightDetection : MonoBehaviour
    {
        private MeshBend bend;
        ...
        private void Awake()
        {
            bend = GetComponent<MeshBend>();
            calculatedPoints = new Vector3[detectionPoints.Length];
        }

        void Start()
        {
            radius = GetComponent<SurfaceTouch>().radius;
        }
        ...
        void Update()
        {
            ...
            for (int i = 0; i < calculatedPoints.Length; i++)
            {
                ...
                calculatedPoints[i] = Bend.CalculatePoint(point, bendDegree);
            }
        }
        ...
        {
            var player = GetComponent<SurfaceTouch>();
